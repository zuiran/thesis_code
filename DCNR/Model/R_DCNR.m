function [accuracy, Alpha0, pre_label0] = R_DCNR(lambda, gamma, beta, train, test)
%
...
%         pre_y = train_descr * solution;
    ...
    %                 errors(j) = norm(y-train_data(:,k)*solution_l1(k,1),2) / norm(solution_l1(k,1),2);
%             errors(j) = norm(y-train_descr(:,k)*solution_l1(k,1),2);   
...
% function y = getSampleDescr(i, descr)
%     y = descr(:, i)
% end
